  ...
  grafanaDashboards+:: {
    'pods.json':
      ...
      local cpuRow = row.new()
        ...
        .addTarget(prometheus.target(
          'sum by (container_name) (rate(container_cpu_usage_seconds_total{%(cadvisorSelector)s, %(clusterLabel)s="$cluster", image!="",container_name!="POD",pod_name="$pod"}[1m]))' % $._config,
      ...
      local networkRow = row.new()
        ...
        .addTarget(prometheus.target(
          'sort_desc(sum by (pod_name) (rate(container_network_receive_bytes_total{%(cadvisorSelector)s, %(clusterLabel)s="$cluster", pod_name="$pod"}[1m])))' % $._config,
